...
def alternative_search_path(start, goal, obstacles, bounds):
    ...
    open_set = []
    # Priority queue stores (heuristic_distance, node) for Greedy Best-First Search
    ...
    while open_set:
        ...
        for neighbor in get_neighbors(current):
            if neighbor not in visited:
                ...
                # Priority is strictly based on heuristic distance to goal
...
while run:
    ...
    if is_alt_path and current_path != "ERROR":
        alt_surf = font.render("Uwaga: Trasa alternatywna (Greedy BFS)", True, (255, 0, 0))
